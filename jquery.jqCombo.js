	...
	*/
	...
	function _autocompleteInput($select, $input, notfoundCss) {
		...
		$input.on('keyup.jqcombo', function(e) {
			// Make entire code of keyup event asynchronous to allow for
			// event to bubble up to document first to get a decent indication
			// of state of pressed keys
			setTimeout(function() {
				...
				if (e.keyCode === keys.tab ||
					($.inArray(e.keyCode, keys.lookback) >= 0 && lastKeycode === keys.tab)) {
					$input.select();
					return;
				}
