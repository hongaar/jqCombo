(function(d){function q(d,n){var a={},c;for(c in n)a[c]=d.css(c);return a}var i={expandOnFocus:!0,expandSize:10,notfoundCss:{color:"red"}},v=[35,36,37,39],x=[38],y=[40],r=[16,17,18],z=[8,46],s=0,m={init:function(b){var n=d.extend({},i,b);if({android:navigator.userAgent.match(/Android/i)?!0:!1,blackberry:navigator.userAgent.match(/BlackBerry/i)?!0:!1,ios:navigator.userAgent.match(/iPhone|iPad|iPod/i)?!0:!1,windows:navigator.userAgent.match(/IEMobile/i)?!0:!1,any:navigator.userAgent.match(/Android|BlackBerry|iPhone|iPad|iPod|IEMobile/i)?
!0:!1}.any)return!1;this.next("select.jqcombo-clone").remove();this.next("input.jqcombo-input").remove();this.off(".jqcombo");return this.each(function(){var a=d(this);a.addClass("jqcombo");var c=d("<input/>");c.css({position:"absolute"});c.addClass("jqcombo-input");c.val(a.find("option:selected").text());a.after(c);var b,e={},j=navigator.userAgent.toLowerCase(),o,i=0,m=[/(chrome)[\/]([\w.]+)/,/(safari)[\/]([\w.]+)/,/(opera)(?:.*version)?[ \/]([\w.]+)/,/(msie) ([\w.]+)/,/(mozilla)(?:.*? rv:([\w.]+))?/],
...
z)==-1){c.val(f);b=e.length;f=f.length;e=c[0];if(e.createTextRange){g=e.createTextRange();g.collapse(true);g.moveStart("character",b);g.moveEnd("character",f);g.select()}else if(e.setSelectionRange)e.setSelectionRange(b,f);else if(e.selectionStart){e.selectionStart=b;e.selectionEnd=f}e.focus()}}else{a.val("");c.css(w)}}});if(n.expandOnFocus){var B=n.expandSize,u;d(window).resize(function(){u={position:"absolute",left:a.position().left,top:a.position().top+a.height()+5,zIndex:1}}).resize();var C=a.attr("size")||
0,D=q(a,u),l=a.clone();l.removeAttr("id name");l.css({visibility:"hidden"}).hide();l.removeClass("jqcombo").addClass("jqcombo-clone");a.after(l);c.on("focus.jqcombo",function(){setTimeout(function(){a.attr("size",B);a.css(u);c.css("z-index",2);l.css({display:"inline-block"})},0)});a.on("focus.jqcombo",function(){p&&clearTimeout(p)});var p,e=function(){p=setTimeout(function(){a.attr("size",C);a.css(D);c.css("z-index","auto");l.hide();p=null},0)};c.on("blur.jqcombo",e);a.on("blur.jqcombo click.jqcombo",
e)}c.on("click.jqcombo",function(){d(this).select()})})}};d.fn.jqCombo=function(b){if(m[b])return m[b].apply(this,Array.prototype.slice.call(arguments,1));if("object"===typeof b||!b)return m.init.apply(this,arguments);d.error("Method "+b+" does not exist on jQuery.jqCombo");return!1}})(jQuery);$.extend($.expr[":"],{startswithi:function(d,q,i){return 0==(d.textContent||d.innerText||"").toLowerCase().indexOf((i[3]||"").toLowerCase())}});